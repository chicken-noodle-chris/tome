...
def test_explicit_vault_wins(make_vault, tmp_path, monkeypatch):
    ...
    resolved = lib.resolve_vault_root(str(vault))
...
def test_explicit_vault_must_have_conventions(tmp_path):
    ...
    with pytest.raises(lib.VaultError, match="conventions.toml"):
        lib.resolve_vault_root(str(not_a_vault))
...
def test_walk_up_finds_parent_vault(make_vault, monkeypatch):
    ...
    resolved = lib.resolve_vault_root(None)
...
def test_nothing_resolves_fails_loud(tmp_path, monkeypatch):
    ...
    with pytest.raises(lib.VaultError, match="no vault found"):
        lib.resolve_vault_root(None)
