...
const resolveTaskId = (id) => ({ href: `/tasks?task=${id}` });
...
describe("linkifyLog", () => {
  ...
  test("a bare task id linkifies to the task panel, case-insensitively", () => {
    ...
    const out = linkifyLog(raw, resolveSlug, resolveTaskId);
    assert.equal(out, "## [2026-07-26] done | [task-83](/tasks?task=task-83): node:test harness over merge.js");
  });

  test("uppercase TASK ids still resolve, keeping the original casing as the label", () => {
    const raw = "See TASK-84 for the follow-up.";
    const out = linkifyLog(raw, resolveSlug, resolveTaskId);
    assert.equal(out, "See [TASK-84](/tasks?task=task-84) for the follow-up.");
  });
  ...
  test("multiple refs on one line all linkify independently", () => {
    ...
    const out = linkifyLog(raw, resolveSlug, resolveTaskId);
    assert.equal(out, "[task-83](/tasks?task=task-83) and [second-brain-framing](/page/second-brain-framing) both mentioned.");
  });
});
